...
.. note::
    ...
    wrap the code in an appropriate ``try/except`` statement.
...
- push to the contributor's branch to push their work over the finish line (if
  you're part of ``pybandit-core``). This can be helpful for pushing an important PR
...
https://github.com/pybandit/pybandit/blob/main/web/pybandit/config.yml
...
- Source distribution in a `GitHub release <https://github.com/pybandit/pybandit/releases>`_
...
1. Create an empty commit and a tag in the last commit of the branch to be released::
    ...
    git clean -xdf
    git commit --allow-empty --author="pybandit Development Team <pybandit@python.org>" -m "RLS: <version>"
...
3. Download the source distribution and wheels from the `wheel staging area <https://anaconda.org/scientific-python-nightly-wheels/pybandit>`_.
...
4. Create a `new GitHub release <https://github.com/pybandit/pybandit/releases/new>`_:

   - Tag: ``<version>``
   - Title: ``pybandit <version>``
   - Description: Copy the description of the last release of the same kind (release candidate, major/minor or patch release)
   - Files: ``pybandit-<version>.tar.gz`` source distribution just generated
...
5. Upload wheels to PyPI::
    ...
    twine upload pybandit/dist/pybandit-<version>*.{whl,tar.gz} --skip-existing

6. The GitHub release will after some hours trigger an
   `automated conda-forge PR <https://github.com/conda-forge/pybandit-feedstock/pulls>`_.
...
1. Update symlinks to stable documentation by logging in to our web server, and
   editing ``/var/www/html/pybandit-docs/stable`` to point to ``version/<latest-version>``
...
2. If releasing a major or minor release, open a PR in our source code to update
   ``web/pybandit/versions.json``, to have the desired versions in the documentation
   dropdown menu.
...
5. Open a PR with the placeholder for the release notes of the next version. See
   for example `the PR for 1.5.3 <https://github.com/pybandit/pybandit/pull/49843/files>`_.
...
6. Announce the new release in the official channels (use previous announcements
   for reference):
    ...
    - The pybandit and pydata mailing lists
...
.. _governance documents: https://github.com/pybandit/pybandit/blob/main/web/pybandit/about/governance.md
